...
static PC1 = [
    ...
    static NUM_OF_LEFT_SHIFTS = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1];

static    chunkString = (str, len) => str.match(new RegExp('.{1,' + len + '}', 'g'));
static hexToBin = (hex) => ("00000000" + (parseInt(hex, 16)).toString(2)).substr(-8);
static decToBin = (dec) => ("0000" + (parseInt(dec, 10)).toString(2)).substr(-4);
static binToHex = (bin) => parseInt(bin, 2).toString(16);
static bin = key => chunkString(key, 2).map(hex => hexToBin(hex)).join("");
static shiftString = (str, shift) => str.slice(shift, str.length) + str.slice(0, shift);
...
static expandBlock = block => E.map(index => block[index - 1]).join("");

static stringXOR = (str1, str2, len) => {
  let xor = Array(len);
  for (let i = 0; i < len; i++) {
...
static encode = (msg, key) => des(msg, key, keySchedule(key));
static decode = (msg, key) => des(msg, key, keySchedule(key).reverse());

//static key = bin("133457799BBCDFF1"); // hex
//static msg = bin("0123456789ABCDEF"); // hex
//static enc = encode(msg, key);
